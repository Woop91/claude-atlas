...
const nodes = [
  ...
  // --- Worklist concepts (from wl_command_reference insights tab) ---
  {
    ...
    description: "If two sessions edit the same unclaimed item simultaneously you get a merge conflict. Claim first with `wl claim`, do the work, then `wl done`. The git-push mechanism acts as an atomic lock: a rejected push means someone else claimed it first — sync and pick another item.",
    tags: ["worklist", "principle"],
    views: ["reference", "worklist"],
  },
  {
    id: "concept.git-push-as-lock",
    kind: "concept",
    domain: "worklist",
    name: "git push is the lock",
    category: "principles",
    badge: { label: "IDEA", hue: 42 },
    oneLine: "Claims are git commits; a rejected push prevents double-booking.",
    description: "The worklist needs no separate daemon. Claims are stored as commits and `git pull --rebase` catches races. A rejected push is the signal that another session claimed the same item — no extra lock file or service required. `wl sync` pushes on every mutation (not batched) to keep parallel sessions from seeing stale state.",
    tags: ["worklist", "principle", "git"],
    views: ["reference", "worklist"],
  },
  {
    id: "concept.worklist-fluency",
    kind: "concept",
    domain: "worklist",
    name: "Worklist fluency",
    category: "principles",
    badge: { label: "IDEA", hue: 42 },
    oneLine: "Map every intent to a wl command rather than an ad-hoc edit.",
    description: "In a worklist repo, always map intent to `wl` commands instead of manual edits. \"Add a task to fix X\" → `wl add`, not a TODO comment. \"What's left?\" → `wl list --status pending`, not a manual scan. Proactive command hints from the coordinator keep parallel sessions fluent and prevent drift.",
    tags: ["worklist", "principle", "workflow"],
    views: ["reference", "worklist"],
  },
  {
    id: "concept.sync-to-share-state",
    kind: "concept",
    domain: "worklist",
    name: "Sync to share state",
    category: "principles",
    badge: { label: "IDEA", hue: 42 },
    oneLine: "Run wl sync after every mutation so all sessions see current state.",
    description: "Because multiple Claude sessions or humans may work in parallel, the shared WORKLIST.md must stay current. `wl sync` runs git pull + push on every mutation — not batched — to minimise the window where a second session sees stale claim data. Always sync after `wl done` before picking your next item.",
    tags: ["worklist", "principle", "sync"],
    views: ["reference", "worklist"],
  },
];
